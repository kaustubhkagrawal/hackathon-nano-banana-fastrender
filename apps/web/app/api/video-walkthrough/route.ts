...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!body.assets || !body.prompt) {
      return NextResponse.json(
        { error: 'Missing required fields: assets, prompt' },
        { status: 400 }
      )
    }
    ...
    // Call the external video walkthrough service
    const response = await fetch(
      'https://windmill.fastrender.app/api/r/hackathon/kaggle-nano-banana-video-walkthrough',
      {
        method: 'POST',
        headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { 
          error: 'Video walkthrough service error',
    ...
    if (error instanceof TypeError && error.message.includes('fetch')) {
      return NextResponse.json(
        { error: 'Failed to connect to video walkthrough service' },
        { status: 502 }
      )
    }
